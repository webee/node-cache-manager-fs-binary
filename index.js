...
DiskStore.prototype.deleting = function deleting(key) {
  ...
  fsp
    .readFile(metaData.filename)
    .then(
      function (metaExtraContent) {
        // delete the files
        if (!metaExtraContent) {
          return
        }
        this.unzipIfNeeded(
          ...
          function (err, data) {
            ...
            if (
              metaExtra.value &&
              metaExtra.value.binary &&
              typeof metaExtra.value.binary === 'object' &&
              metaExtra.value.binary != null
            ) {
              // unlink binaries
              async.forEachOf(
                metaExtra.value.binary,
            ...
            [
              ...
              function (diskdata, seriescb) {
                if (
                  diskdata &&
                  diskdata.value &&
                  diskdata.value.binary &&
                  diskdata.value.binary != null &&
                  typeof diskdata.value.binary == 'object'
                ) {
                  async.forEachOf(
                    diskdata.value.binary,
                    function (v, k, cb) {
